...
bool is_square_attacked(const Position& pos, int square, Color attacker_color, U64 custom_occ);

// direct checks only
bool gives_check(const Position& pos, Move move);

// pos is not constant becasue of en passant make unmake
...
int generate_legal_captures(Position& pos, const KingSafety& safety, Move* moves);
