...
// magic bitboard sliding piece attacks, pseudo-legal and does not check for
// legality
inline U64 rook(int square, U64 occupancy) {
  return Magics::rook_attacks(square, occupancy);
}
inline U64 bishop(int square, U64 occupancy) {
  return Magics::bishop_attacks(square, occupancy);
}
