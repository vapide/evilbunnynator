...
constexpr Move encode_move(int from_sq, int to_sq, int flags = 0, int promotion = 0) {
    return static_cast<Move>(from_sq | (to_sq << 6) | (flags << 12) | (promotion << 16));
}
...
// against the legal list which makes a hash collision harmless, a foreign move16 simply matches nothing
