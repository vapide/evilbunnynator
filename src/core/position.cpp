...
Position Position::from_fen(const std::string& fen) {
    ...
    if (castling.find('K') != std::string::npos) position.castling_rights |= WK_CASTLE;
    if (castling.find('Q') != std::string::npos) position.castling_rights |= WQ_CASTLE;
    if (castling.find('k') != std::string::npos) position.castling_rights |= BK_CASTLE;
    if (castling.find('q') != std::string::npos) position.castling_rights |= BQ_CASTLE;
...
// basically ported from position.py, with minor changes due to the nature of C++ and optimizations.
void Position::make_move(Move move) {
    ...
    state.removed[state.removed_count++] = {moving_piece, static_cast<Square>(from_sq)};
    ...
    if (captured_piece != NO_PIECE) {
        ...
        state.removed[state.removed_count++] = {captured_piece, static_cast<Square>(captured_square)};
    }

    move_piece(moving_piece, from_sq, to_sq);

    if ((moving_piece == WK || moving_piece == BK) && std::abs(from_sq - to_sq) == 2) {
        if (moving_piece == WK) {
            if (to_sq == G1) {
                move_piece(WR, H1, F1);
    ...
    if (promotion != PROMO_NONE && is_pawn) {
        static constexpr PieceType promo_types[5] = {
            PAWN /*unused*/, KNIGHT, BISHOP, ROOK, QUEEN
        };
        Piece promoted_piece = static_cast<Piece>(
            promo_types[promotion] + (moving_piece == WP ? 0 : 6));

        // not toggling pawn at to_sq, the key was already removed at from_sq and at to_sq it was never hashed.
        // python position.py had a toggle which leaves a random pawn key xored into the after-promotion hash 
        // which corrupts tt/repetition.
        ...
        state.added[state.added_count++] = {promoted_piece, static_cast<Square>(to_sq)};
        state.promotion_piece = promoted_piece;
    } else {
        zobrist.toggle_piece(moving_piece, static_cast<Square>(to_sq));
        state.added[state.added_count++] = {moving_piece, static_cast<Square>(to_sq)};
    }
...
void Position::unmake_move() {
    ...
    } else if ((moving_piece == WK || moving_piece == BK) && std::abs(from_sq - to_sq) == 2) {
        move_piece(moving_piece, to_sq, from_sq);
...
bool Position::is_insufficient_material() const {
    ...
    if (pieces[WP] | pieces[BP] | pieces[WR] | pieces[BR] | pieces[WQ] | pieces[BQ])
        return false;
